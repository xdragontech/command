...
import { deleteSponsorTier, updateSponsorTier } from "@command/core-partners";
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (!id) return json(res, 400, { ok: false, error: "Sponsor tier id is required" });
  ...
  try {
    ...
    return json(res, 400, { ok: false, error: error?.message || "Failed to manage sponsor tier" });
  }
}
