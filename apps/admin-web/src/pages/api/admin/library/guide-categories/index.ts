...
import { createContentGuideCategory, listContentGuideCategories } from "@command/core-content";
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    if (req.method === "GET") {
      const categories = await listContentGuideCategories({
        scope: {
    ...
    if (req.method === "POST") {
      ...
      const category = await createContentGuideCategory(
        {
          role: auth.principal.role,
          allowedBrandIds: auth.principal.allowedBrandIds,
        },
