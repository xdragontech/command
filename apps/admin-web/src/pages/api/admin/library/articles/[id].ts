...
import { deleteContentArticle, updateContentArticle } from "@command/core-content";
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    if (req.method === "PUT") {
      ...
      const article = await updateContentArticle(
        {
      ...
      return json(res, 200, { ok: true, article });
    }

    if (req.method === "DELETE") {
      await deleteContentArticle(
        {
          role: auth.principal.role,
          allowedBrandIds: auth.principal.allowedBrandIds,
        },
    ...
    const status =
      message === "Forbidden brand scope" ? 403 : message === "Article not found" ? 404 : 400;
