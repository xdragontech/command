...
import { deleteContentGuide, updateContentGuide } from "@command/core-content";
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    if (req.method === "PUT") {
      ...
      const guide = await updateContentGuide(
        {
    ...
    const status =
      message === "Forbidden brand scope" ? 403 : message === "Guide not found" ? 404 : 400;
