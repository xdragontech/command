...
import { deleteContentPrompt, updateContentPrompt } from "@command/core-content";
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    if (req.method === "PUT") {
      ...
      const prompt = await updateContentPrompt(
        {
    ...
    const status =
      message === "Forbidden brand scope" ? 403 : message === "Prompt not found" ? 404 : 400;
