...
import { deleteContentCategory, updateContentCategory } from "@command/core-content";
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    if (req.method === "PUT") {
      ...
      const category = await updateContentCategory(
        {
    ...
    const status =
      message === "Forbidden brand scope" ? 403 : message === "Category not found" ? 404 : 400;
