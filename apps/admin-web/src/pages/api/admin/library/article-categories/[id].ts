...
import { deleteContentArticleCategory, updateContentArticleCategory } from "@command/core-content";
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    if (req.method === "PUT") {
      ...
      const category = await updateContentArticleCategory(
        {
    ...
    if (req.method === "DELETE") {
      await deleteContentArticleCategory(
        {
          role: auth.principal.role,
          allowedBrandIds: auth.principal.allowedBrandIds,
        },
    ...
    const status =
      message === "Forbidden brand scope" ? 403 : message === "Article category not found" ? 404 : 400;
