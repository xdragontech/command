...
import { createContentArticleCategory, listContentArticleCategories } from "@command/core-content";
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    if (req.method === "GET") {
      const categories = await listContentArticleCategories({
        scope: {
    ...
    if (req.method === "POST") {
      ...
      const category = await createContentArticleCategory(
        {
          role: auth.principal.role,
          allowedBrandIds: auth.principal.allowedBrandIds,
        },
