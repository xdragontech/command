...
import { createScheduleParticipant, listScheduleParticipants } from "@command/core-scheduling";
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    if (req.method === "GET") {
      ...
      return json(res, 200, { ok: true, participants });
    }

    if (req.method === "POST") {
      const body = typeof req.body === "string" ? JSON.parse(req.body || "{}") : req.body || {};
      const participant = await createScheduleParticipant({
        scope: toSchedulingScope(auth.principal),
        input: body,
      });
