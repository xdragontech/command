...
import {
  deleteSchedulePublicFeed,
  updateSchedulePublicFeed,
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (!id) return json(res, 400, { ok: false, error: "Feed id is required" });

  try {
    if (req.method === "PATCH") {
      const body = typeof req.body === "string" ? JSON.parse(req.body || "{}") : req.body || {};
      const feed = await updateSchedulePublicFeed({
    ...
    if (req.method === "DELETE") {
      await deleteSchedulePublicFeed({
        scope: toSchedulingScope(auth.principal),
        id,
      });
