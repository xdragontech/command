...
import {
  deleteScheduleEventSeries,
  updateScheduleEventSeries,
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (!id) return json(res, 400, { ok: false, error: "Series id is required" });

  try {
    if (req.method === "PATCH") {
      const body = typeof req.body === "string" ? JSON.parse(req.body || "{}") : req.body || {};
      const series = await updateScheduleEventSeries({
    ...
    if (req.method === "DELETE") {
      await deleteScheduleEventSeries({
        scope: toSchedulingScope(auth.principal),
        id,
      });
